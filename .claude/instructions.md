...
**Folder structure**: `models/staging/kroger_unnnested_raw_data/` holds the two "close to raw" layers (Layer 1 `stg_kroger_*`, Layer 2 `stg_json_kroger_*`, plus `_models.yml` and `__sources.yml`); `models/staging/` directly holds the 21 Layer 3 array fan-out models (`stg_json_kroger_{pricing,inventory}_items`, `_images`, `_nutrition_information`, etc., plus `stg_json_kroger_locations_departments`). Full reasoning + lineage diagrams in `Project Architecture.md`. `models/marts/` is currently empty — the original dbt-init tutorial content (customers/orders/products) was deleted as unrelated boilerplate.
