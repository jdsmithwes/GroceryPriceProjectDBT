...
**What's not built yet.** This section predates the Orchestration pipeline (2026-08-23) — see `.claude/instructions.md` and `Orchestration/README.md` for current state; not rewritten fully here yet. As of 2026-08-23, Kroger runs on a real weekly schedule (EventBridge Scheduler + Fargate, see `Orchestration/README.md`) — collection is no longer purely manual — and as of 2026-09-12 neither is the dbt refresh: the Kroger hot-path models are Snowflake dynamic tables that refresh themselves after each load (see `.claude/instructions.md` → Materialization strategy). dbt's Kroger staging layer (below) is fully built, plus an intermediate layer collapsing it into actual price history — but marts, the layer that joins catalog/pricing/inventory/location together and applies business logic, still doesn't exist. Walmart's ingestion script and Snowflake landing table/pipe both work as of 2026-08-31 (see `.claude/instructions.md`), and it runs as a second leg of the same weekly schedule (since 2026-09-04). A cross-retailer category bridge (`int_retailer_product_categories`, 2026-09-12) maps Kroger and Walmart categories onto one shared list. Aldi and Publix, briefly part of this project, were removed entirely 2026-08-31 — Aldi's API host never resolved and no replacement was found; Publix was never live-verified. The forecasting model itself (Phase 5) is not yet designed — it depends on enough historical pricing snapshots accumulating in `KROGER_PRICING`, which the new schedule is now actually building toward instead of relying on ad hoc runs.
