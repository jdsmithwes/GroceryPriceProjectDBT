...
-- GROCERYDBTPROJECT.RAW.KROGER_INVENTORY. raw_data here is the FULL
-- Kroger product object (same shape as stg_json_kroger_pricing, since
-- both pipelines land the identical raw response — see the NOTE in
-- Kroger_Inventory_*.py). LOCATION_ID is kept from the outer staging
-- model since it's NOT part of the product JSON itself (it's the store
-- this pull was scoped to, tracked separately by the pipeline).
--
...
with source as (
    ...
    select * from {{ ref('stg_kroger_inventory') }}
