...
def load_stores():
    ...
    return session.sql("""
        SELECT DISTINCT
            ...
            l.STATE,
            l.ZIP_CODE
        FROM GROCERYDBTPROJECT.GROCERY_STAGING.STG_JSON_KROGER_LOCATIONS l
        WHERE l.LOCATION_ID IS NOT NULL AND l.NAME IS NOT NULL
          AND l.LOCATION_ID IN (
              SELECT DISTINCT LOCATION_ID
              FROM GROCERYDBTPROJECT.GROCERY_INTERMEDIATE.INT_KROGER_PRICE_HISTORY
...
def load_products(search_term, brand_filter, limit, location_id, price_only):
    query = """
        SELECT
            ...
            s.IN_STORE,
            s.SHIP_TO_HOME
        FROM GROCERYDBTPROJECT.GROCERY_STAGING.STG_KROGER_PRODUCT_CATALOG p
        LEFT JOIN GROCERYDBTPROJECT.GROCERY_INTERMEDIATE.INT_KROGER_PRICE_HISTORY ph
        ...
        LEFT JOIN (
            SELECT
                ...
                IN_STORE,
                SHIP_TO_HOME
            FROM GROCERYDBTPROJECT.GROCERY_STAGING.STG_JSON_KROGER_PRODUCT_SNAPSHOT_ITEMS
            WHERE LOCATION_ID = ?
...
def load_brands():
    df = session.sql("""
        ...
        FROM GROCERYDBTPROJECT.GROCERY_STAGING.STG_KROGER_PRODUCT_CATALOG
