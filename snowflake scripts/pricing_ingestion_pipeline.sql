...
CREATE TABLE IF NOT EXISTS GROCERYDBTPROJECT.RAW.KROGER_PRICING (
  PRODUCTID STRING,
  LOCATIONID STRING,
...
ALTER TABLE GROCERYDBTPROJECT.RAW.KROGER_PRICING
...
-- files.
CREATE OR REPLACE PIPE GROCERYDBTPROJECT.AWS_RESOURCES.KROGER_PRICING_PIPE
  AUTO_INGEST = TRUE
AS
  COPY INTO GROCERYDBTPROJECT.RAW.KROGER_PRICING
...
-- locations, and inventory pipes' queues. Add it as an ADDITIONAL S3
-- Event Notification on the bucket for the kroger/ prefix (all
-- object-create events) — do not remove or replace the other pipes'
-- queue registrations; all four need to coexist on the same prefix, each
