...
-- [RUN ONCE] NOTE: this project's raw schema has been renamed twice —
-- RAW -> GROCERY_RAW (after 2026-08-11) -> split into GROCERY_RAW_KROGER
-- / GROCERY_RAW_WALMART (2026-09-01, once Walmart landing existed
-- alongside Kroger's and the two needed separating; see
-- .claude/instructions.md's Snowflake section). AWS_RESOURCES was split
-- the same day into AWS_RESOURCES_KROGER / AWS_RESOURCES_WALMART for the
-- per-source stages/pipes, while genuinely shared objects (file formats,
-- the root S3 stage) stayed in AWS_RESOURCES. This line is kept only as
-- a historical record of what originally ran — don't create a literal
-- GROCERY_RAW schema from this; use the per-source schemas below instead.
CREATE SCHEMA IF NOT EXISTS GROCERYDBTPROJECT.GROCERY_RAW_KROGER;
CREATE SCHEMA IF NOT EXISTS GROCERYDBTPROJECT.GROCERY_RAW_WALMART;
CREATE SCHEMA IF NOT EXISTS GROCERYDBTPROJECT.AWS_RESOURCES;
CREATE SCHEMA IF NOT EXISTS GROCERYDBTPROJECT.AWS_RESOURCES_KROGER;
CREATE SCHEMA IF NOT EXISTS GROCERYDBTPROJECT.AWS_RESOURCES_WALMART;
...
CREATE OR REPLACE STAGE GROCERYDBTPROJECT.AWS_RESOURCES_KROGER.MY_S3_STAGE_KROGER
  STORAGE_INTEGRATION = GROCERY_PRICE_PROJECT
...
FROM TABLE(
  INFER_SCHEMA(
    LOCATION => '@GROCERYDBTPROJECT.AWS_RESOURCES_KROGER.MY_S3_STAGE_KROGER',
    FILE_FORMAT => 'GROCERYDBTPROJECT.AWS_RESOURCES.MY_CSV_INFER'
  )
);
...
CREATE OR REPLACE TABLE GROCERYDBTPROJECT.GROCERY_RAW_KROGER.KROGER_PRODUCT_CATALOG
  USING TEMPLATE (
...
ALTER TABLE GROCERYDBTPROJECT.GROCERY_RAW_KROGER.KROGER_PRODUCT_CATALOG
  ADD COLUMN IF NOT EXISTS INGESTED_FILENAME STRING;

-- 4. [RUN ONCE — then AUTOMATIC] Snowpipe: auto-loads any new file that
-- lands under kroger/ into GROCERY_RAW_KROGER.KROGER_PRODUCT_CATALOG. Once this exists
...
CREATE OR REPLACE PIPE GROCERYDBTPROJECT.AWS_RESOURCES_KROGER.KROGER_PRODUCT_CATALOG_PIPE
  AUTO_INGEST = TRUE
AS
  COPY INTO GROCERYDBTPROJECT.GROCERY_RAW_KROGER.KROGER_PRODUCT_CATALOG
...
CREATE OR REPLACE STAGE GROCERYDBTPROJECT.AWS_RESOURCES_WALMART.MY_S3_STAGE_WALMART
  STORAGE_INTEGRATION = GROCERY_PRICE_PROJECT
...
FROM TABLE(
  INFER_SCHEMA(
    LOCATION => '@GROCERYDBTPROJECT.AWS_RESOURCES_WALMART.MY_S3_STAGE_WALMART',
    FILE_FORMAT => 'GROCERYDBTPROJECT.AWS_RESOURCES.MY_CSV_INFER',
...
CREATE OR REPLACE TABLE GROCERYDBTPROJECT.GROCERY_RAW_WALMART.WALMART_PRODUCT_CATALOG (
  itemId VARCHAR,
...
-- 3b. [RUN ONCE] Add a column to hold which source file each row came from.
ALTER TABLE GROCERYDBTPROJECT.GROCERY_RAW_WALMART.WALMART_PRODUCT_CATALOG
  ADD COLUMN IF NOT EXISTS INGESTED_FILENAME STRING;

-- 4. [RUN ONCE — then AUTOMATIC] Snowpipe: auto-loads any new file
-- matching PATTERN under walmart/ into GROCERY_RAW_WALMART.WALMART_PRODUCT_CATALOG.
...
CREATE OR REPLACE PIPE GROCERYDBTPROJECT.AWS_RESOURCES_WALMART.WALMART_PRODUCT_CATALOG_PIPE
  AUTO_INGEST = TRUE
AS
  COPY INTO GROCERYDBTPROJECT.GROCERY_RAW_WALMART.WALMART_PRODUCT_CATALOG
  FROM @GROCERYDBTPROJECT.AWS_RESOURCES_WALMART.MY_S3_STAGE_WALMART
